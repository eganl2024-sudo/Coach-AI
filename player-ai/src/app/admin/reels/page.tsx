...
import ReelReviewFilterTabs from '@/components/admin/ReelReviewFilterTabs';
...
export default async function AdminReelsPage({ searchParams }: Props) {
  ...
  const reels = await getAllSubmittedReels(reviewer || undefined);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 flex gap-x-2">
            ...
            {reviewer && <span className="text-muted-foreground">· filtered</span>}
          </p>
        </div>
        <Suspense>
          <ReelReviewFilterTabs />
        </Suspense>
      </div>
      ...
      {reels.length === 0 ? (
        <div className="border border-border/50 rounded-xl bg-card/40 py-16 text-center">
          <p className="text-muted-foreground text-sm">
            {reviewer ? 'No submissions for this reviewer yet.' : 'No submissions yet.'}
          </p>
        </div>
      ) : (
