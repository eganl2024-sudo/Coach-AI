...
import { addDrillAction } from '@/lib/actions/admin';
...
export default function DrillAddForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.drill_id || !formData.drill_id.trim()) {
      setError('Drill ID is required.');
      return;
    }
    if (!formData.drill_name || !formData.drill_name.trim()) {
    ...
    setSaving(true);
    try {
      const res = await addDrillAction(formData);
      if (res.success && res.drill_id) {
        router.push(`/admin/drills/${res.drill_id}/edit`);
        router.refresh();
      } else {
        setError(res.error || 'Failed to add new drill.');
      }
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 pb-20">
      ...
      <Card className="border-border/50 bg-card/40 backdrop-blur-sm p-4">
        <CardContent className="p-0 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <FieldLabel>Drill ID (Unique Primary Key) *</FieldLabel>
              <Input
                type="text"
                placeholder="e.g. DRILL_100"
                value={formData.drill_id || ''}
                onChange={e => handleChange('drill_id', e.target.value.trim())}
                required
                className="bg-card/30"
              />
            </div>
      ...
      <Button
        type="submit"
        disabled={saving}
        className="w-full h-11 text-sm font-semibold rounded-lg shadow-lg bg-primary hover:bg-primary/95 text-primary-foreground shadow-primary/10 transition-all cursor-pointer"
      >
        {saving ? 'Creating...' : 'Create Drill'}
      </Button>
    </form>
  );
}
