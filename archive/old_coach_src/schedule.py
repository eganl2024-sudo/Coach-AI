...
# Flexible date: 1-2 digits for month/day, 2 or 4 digits for year, / or - separators
DATE_RE = r"(?P<date>\d{1,2}[/-]\d{1,2}[/-](?:\d{2}|\d{4}))"

# Flexible time: 12h or 24h, optional minutes, optional space, optional AM/PM with/without periods
TIME_RE = r"(?P<time>\d{1,2}(?::\d{2})?(?:\s*[aApP][mM]\.?)?)"

# Flexible event type: Practice, Game, Match, Training, etc.
# Non-capturing group for the prefix, capturing group for the value
TYPE_RE = r"(?:\s+(?P<etype>Practice|Game|Match|Training|Scrimmage|Event))?"

# The lookahead must match the SAME date format to stop correctly
LOOKAHEAD_RE = r"(?=(\s*\d{1,2}[/-]\d{1,2}[/-](?:\d{2}|\d{4}))|\Z)"

EVENT_PATTERN = re.compile(
    f"{DATE_RE}\\s+{TIME_RE}{TYPE_RE}\\s+(?P<rest>.*?){LOOKAHEAD_RE}",
...
def _normalize_date(date_str: str) -> str:
    ...
    for fmt in ("%m/%d/%Y", "%Y-%m-%d", "%m/%d/%y", "%m-%d-%Y", "%m-%d-%y"):
        try:
            ...
            continue
    # Try parsing fuzzy dates if needed, or return raw
    return date_str
